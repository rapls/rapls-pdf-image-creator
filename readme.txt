...
= 1.4.0 =
When a thumbnail fails to generate, the Status tab now tells you why instead of leaving you with a PDF and no picture. A page that comes back blank is retried with a workaround for a known Ghostscript fault, and refused rather than saved as a white image if it is still blank. Also checks whether picking a page other than the first actually works on your server.
